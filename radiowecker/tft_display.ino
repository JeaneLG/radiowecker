  ...
  if (clockmode) { //if we are in the clock mode, we switch into the radiopage or adjust Gain
    ...
    if (p.y > 180) { //if the y value > 180, we are in the button area
      ...
      if (p.x < 64) {
        ...
        if (alarmPause) alarmPause = false; // if alarm is paused switch it completely off
        // back to main page
        clockmode = true;
        configpage = false;
        radiopage = false;
...
void setSnoozeTime(uint16_t value) {
  ...
  displayMessage(231,96,80,20,txt,ALIGNRIGHT,false,ILI9341_BLACK,COLOR_SETTING_BG,1);
}
...
void toggleAlarm() {
  if (alarmday > 7) {
    //if the alarm was off find the next alarm and activate it
    findNextAlarm();
...
void startSnooze() {
  ...
  snoozeEndTime = minutes + snoozeTime;
  toggleRadio(false);
  clockmode = true;
  showClock();
}
...
void textInBox(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const char* text, uint8_t align, boolean big, uint16_t fc, uint16_t bg, uint8_t lines) {
  char tmp[256];
  char msg[50];
  char * token;
...
void drawHeaderInfos(){
  ...
  drawAlarmPauseInfo();// Alarm Pause symbol zeichnen
...
void drawWifiInfo() {
    ...
    textInBox(92, 0, 159, 17, localIPString.c_str(), ALIGNCENTER, false, COLOR_IP, COLOR_BG);
}
...
void showSlider(uint16_t y, float value, uint16_t vmax, uint16_t bgColor, uint16_t lineColor) {
    tft.fillRect(11, y, 299, 14, bgColor);
...
void showRadioPage() {
    ...
    //Back to main page
    if (alarmOn || alarmPause) tft.drawBitmap(0,176,knopf_sym[0],64,64,ILI9341_RED,COLOR_KNOEPFE_BG);
...
void showNextAlarm(){
  ...
  if (clockmode) {
    ...
    if (alarmPause) {
      color_alarm = COLOR_ALARM_PAUSE_SYMBOL;
      symbol = 2;//bell_17
      h = alarmResumeTime / 60;
      m = alarmResumeTime % 60;
      sprintf(txt,"%s %02i:%02i",days_short[weekday],h,m);      
    }
